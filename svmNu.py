...
with open("Resultado.txt", "w") as file1:
    ...
    modelSVMNu = NuSVC(kernel = 'precomputed')
    ...
    resultSVM = cross_val_score(modelSVMNu, data, labels, cv = skf)
    #resultRFC = cross_val_score(modelRFC, data, labels, cv = skf)
    file1.write("Accuracy SVMNu: %0.4f (+/- %0.2f)" % (resultSVM.mean(), resultSVM.std() * 2))
    resultsList.append(resultSVM.mean())
    #resultsList.append(resultRFC.mean())
    #print do resultado
    print("Accuracy SVM precomputed: %0.4f (+/- %0.2f)" % (resultSVM.mean(), resultSVM.std() * 2))
